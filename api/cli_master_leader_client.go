...
// to the caller.
func MasterLeaderConnectedClient(commandConfig *configs.CliConfig, logger hclog.Logger) (YBClientAPI, error) {

	validConfigs := map[string]*ybClientConfigs.YBClientConfig{}
	...
	chanConnectedClient := make(chan YBClientAPI, 1)
	chanErrors := make(chan error)

	for hostPort, cliConfig := range validConfigs {
		go func(thisHostPort string, thisConfig *ybClientConfigs.YBClientConfig) {
